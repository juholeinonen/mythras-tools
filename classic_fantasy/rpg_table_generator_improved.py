...
def calculate_probability_ranges(config: Dict, total_range: int = 100) -> List[Tuple[str, str, str]]:
    ...
    Fixed to ensure ranges go to 100, not 90.
    """
    results = []
    current_start = 1
    ...
    # Track remaining range to distribute
    remaining_range = total_range
    items_list = list(config.items())
    
    for i, (main_type, data) in enumerate(items_list):
        weight = data.get('weight', 0)
        
        # For the last item, use all remaining range to ensure we hit 100
        if i == len(items_list) - 1:
            main_range_size = remaining_range
        else:
            main_range_size = round((weight / total_weight) * total_range)
            remaining_range -= main_range_size
        ...
        if subtypes:
            ...
            subtype_items = list(subtypes.items())
            subtype_remaining = main_range_size
            
            for j, (subtype, subtype_weight) in enumerate(subtype_items):
                # For the last subtype, use all remaining range
                if j == len(subtype_items) - 1:
                    subtype_size = subtype_remaining
                else:
                    subtype_size = round((subtype_weight / subtype_total_weight) * main_range_size)
                    subtype_remaining -= subtype_size
